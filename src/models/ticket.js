  ...
  if (!this.wasNew) {
    ...
    try {
      const savedTicket = await doc.populate([
        {
          path: 'owner assignee comments.owner notes.owner subscribers history.owner timeEntries.owner',
...
ticketSchema.statics.getAllByStatus = function (status, callback) {
  ...
  const q = self
    ...
    .populate({
      path: 'owner assignee requester comments.owner notes.owner subscribers history.owner timeEntries.owner',
      select: 'username fullname email role image title'
    })
    .populate('type tags status group product module')
    .sort({ status: 1 })
    .lean()
...
function buildQueryWithObject (SELF, grpId, object, count) {
  ...
  if (count) query = SELF.model(COLLECTION).countDocuments({ groups: { $in: grpId }, deleted: false })
    ...
    query = SELF.model(COLLECTION)
      ...
      .populate({
        path: 'owner assignee requester subscribers comments.owner notes.owner history.owner timeEntries.owner',
        select: 'username fullname email role image title'
      })
      .populate('type tags status group product module')
      .sort({ uid: -1 })
