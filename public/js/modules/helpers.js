...
define([
  ...
  helpers.getViewDataItem = function (key) {
    // Try to get from window.__viewdata first (if set by server)
    if (typeof window.__viewdata !== 'undefined' && window.__viewdata && typeof window.__viewdata[key] !== 'undefined') {
      return window.__viewdata[key]
    }

    // Try to get from trudeskSettingsService
    if (typeof window.trudeskSettingsService !== 'undefined' && window.trudeskSettingsService) {
      const settings = window.trudeskSettingsService.getSettings()
    ...
    // Try to get from DOM element (as fallback for certain values)
    const $elem = $('#__' + key)
    if ($elem.length > 0) {
